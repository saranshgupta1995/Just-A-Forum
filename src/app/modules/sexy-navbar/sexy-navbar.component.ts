...
export class SexyNavbarComponent implements OnInit {
    ...
    @ViewChild('content') content: ModalLoginComponent;
    ...
    socialFormViewState = 'inactive'
    socialFormView = false;
    animTime = 200;
    errorState = false;
    loggedIn = false;
    ...
    toggleToSocial() {
        ...
        let that = this
        setTimeout(() => {
            that.socialFormView = !that.socialFormView;
            // that.socialFormViewState = that.socialFormViewState === 'active' ? 'inactive' : 'active';
        }, that.animTime)
        setTimeout(() => { that.content.open() }, that.animTime * 2)
    }
